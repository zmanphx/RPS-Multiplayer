  ...
  if (prsArr.indexOf(p1Pick) > -1 && prsArr.indexOf(p2Pick) > -1) {
    ...
    database.ref("Players/player2").update({ClickVal:""});
      ...
      if (p1Pick === p2Pick) {
         ...
         var dispText=  response.player1.Name +" chose "+ rpsTranslate(p1Pick) + ", " + response.player2.Name +" chose " +
         rpsTranslate(p2Pick) + "  Tie!!!";
        ...
        (p1Pick === "p" && p2Pick === "s")
           ...
           var dispText=  response.player1.Name +" chose "+ rpsTranslate(p1Pick) + ", " + response.player2.Name +" chose " +
           rpsTranslate(p2Pick) + ": " + response.player2.Name + " Wins!!";
           $("#idHolder1").text(dispText);
        ...
        var dispText=  response.player1.Name +" chose "+ rpsTranslate(p1Pick) + ", " + response.player2.Name +" chose " +
        rpsTranslate(p2Pick) + ": " + response.player1.Name + " Wins!!";
        $("#idHolder1").text(dispText);
...
function rpsTranslate(pick){
  ...
  if (pick=="p") { return "paper";}
    else
    if (pick=="s") { return "scissors";}
    else
    if (pick =="r"){return "rock";}
}
